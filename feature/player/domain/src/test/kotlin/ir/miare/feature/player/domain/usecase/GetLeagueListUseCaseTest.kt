...
internal class GetLeagueListUseCaseTest {
    ...
    fun `given repository returns success, when use case is invoked, then result is success`() =
        runTest {
            ...
            coEvery { repository.getLeagues(1, 10) } returns Result.Success(expected)

            // Act
            val result = getLeagueListUseCase(1, 10)

            // Assert
            result shouldBe Result.Success(expected)
            coVerify(exactly = 1) { repository.getLeagues(1, 10) }
        }
    ...
    fun `given repository returns network error, when use case is invoked, then result is failure`() =
        runTest {
            ...
            coEvery { repository.getLeagues(2, 20) } returns Result.Failure(error)

            // Act
            val result = getLeagueListUseCase(2, 20)

            // Assert
            result shouldBe Result.Failure(error)
            coVerify(exactly = 1) { repository.getLeagues(2, 20) }
        }
    ...
    fun `given repository returns empty list, when use case is invoked, then result is success with empty data`() =
        runTest {
            // Arrange
            val emptyResult = PaginatedResult<League>(data = emptyList(), totalPages = 1)
            coEvery { repository.getLeagues(1, 10) } returns Result.Success(emptyResult)
    ...
    fun `given repository called with multiple pages, when use case invoked sequentially, then results accumulate correctly`() =
        runTest {
            ...
            val page2 = PaginatedResult(
                data = listOf(League("Premier League", "England", 2, 38)),
                totalPages = 2
            )
            coEvery { repository.getLeagues(1, 10) } returns Result.Success(page1)
            coEvery { repository.getLeagues(2, 10) } returns Result.Success(page2)

            // Act
            val result1 = getLeagueListUseCase(1, 10)
            val result2 = getLeagueListUseCase(2, 10)

            // Assert
            result1 shouldBe Result.Success(page1)
            result2 shouldBe Result.Success(page2)
            coVerify(exactly = 1) { repository.getLeagues(1, 10) }
            coVerify(exactly = 1) { repository.getLeagues(2, 10) }
        }

    @Test
    fun `given repository throws exception, when use case invoked, then exception propagates`() = runTest {
        // Arrange
        val exception = RuntimeException("Unexpected")
        coEvery { repository.getLeagues(1, 10) } throws exception

        // Act & Assert
        val thrown = shouldThrow<RuntimeException> {
            getLeagueListUseCase(1, 10)
        }
